...
class MaskedAutoencoder(nn.Module):
    def __init__(self):
        ...
        self.encoder_conv = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(64, 32, 3, stride=2, padding=1),         # 16 → 8
            nn.ReLU(),
            nn.Conv2d(32, 16, 3, stride=2, padding=1),         # 8 → 4 
            nn.ReLU(),
        )
        # latent shape = (batch, 16, 4, 4)  →  256 scalars per sample

        self.decoder_conv = nn.Sequential(
            nn.ConvTranspose2d(16, 32, 3, stride=2, padding=1, output_padding=1),   # 4 → 8 
            nn.ReLU(),
            nn.ConvTranspose2d(32, 64, 3, stride=2, padding=1, output_padding=1),   # 8 → 16
            nn.ReLU(),
