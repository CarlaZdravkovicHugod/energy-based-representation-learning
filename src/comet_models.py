...
def kaiming_init(m):
    '''
    This function initializes the weights of a layer with the Kaiming Normal initialization.
    This initialization is designed to keep the variance of the input and output weights the same.
    '''
...
class CondResBlock(nn.Module):
    '''This class defines the conditional residual block used in the LatentEBM model.
    The block consists of two convolutional layers with Instance Normalization and Swish activation functions.
    The block also has an option to downsample the input image and rescale the latent vector.
    '''
...
class CondResBlockNoLatent(nn.Module):
    ...
    The block consists of two convolutional layers with Instance Normalization and Swish activation functions.
    The block also has an option to downsample the input image and rescale the latent vector.
    However, this block does not use the latent vector.
    '''
...
class BroadcastConvDecoder(nn.Module):
    '''This class defines the broadcast convolutional decoder used in the LatentEBM model.
    The decoder takes the latent vector and broadcasts it to the size of the image.
    The decoder then applies a series of convolutional layers to generate the output image.
    '''
...
class LatentEBM128(nn.Module):
    ...
    The model takes an image and a latent vector as input and outputs the energy of the image.
    '''
...
class LatentEBM(nn.Module):
    ...
    The model consists of a series of conditional residual blocks and a broadcast convolutional decoder.
    The model takes an image and a latent vector as input and outputs the energy of the image.
    The difference between this model and the LatentEBM128 model is that this model uses a different number of filters.'''
    def __init__(self, args, dataset):
        ...
        self.embed_conv1 = nn.Conv2d(3, filter_dim, kernel_size=3, stride=1, padding=1) # TODO: get channels from imshape,  MRI scans have 12 channels because of the 3D thing
